...
# We consider the 1D nonlinear damped wave problem
# u_{tt}+a'(u)u_{t}-u_{txx}-(b(u))_{xx} =f with a(u)=u+u^3, b(u)=u+u^3
...
L= np.identity(d)+fractional_matrix_power(M_array,-1/2)@ (K_array @fractional_matrix_power(M_array,-1/2))
...
for it in range(1,nt):
    ...
    def delta(i,j):
        if i== j:
            ...
            return 0
    M2_bar=[[[[quad(lambda t: int_t(t,i,j,m,n), time[it-1], time[it])[0] for j in range (0,4)] for i in range(0,4)] for n in range(0,4) ] for m in range(0,4)]
    ...
    Gexpt1=(2*(np.pi**2)+2)*(fractional_matrix_power(M_array,-1/2)@ F1_array)
    ...
    +K23_bar[i,j]*(M3_bar[1][2]+M5_bar[1][2])+K24_bar[i,j]*(M3_bar[1][3]+M5_bar[1][3])+K34_bar[i,j]*(M3_bar[2][3]+M5_bar[2][3])+np.multiply(M2_bar[0][0],M11_bar[i,j])\
    +np.multiply(M2_bar[1][1],M22_bar[i,j])+np.multiply(M2_bar[2][2],M33_bar[i,j])+np.multiply(M2_bar[3][3],M44_bar[i,j]) + np.multiply(M2_bar[0][1],M12_bar[i,j])\
    +np.multiply(M2_bar[0][2],M13_bar[i,j])+np.multiply(M2_bar[0][3],M14_bar[i,j])+np.multiply(M2_bar[1][2],M23_bar[i,j])+np.multiply(M2_bar[1][3],M24_bar[i,j])\
    +np.multiply(M2_bar[2][3],M34_bar[i,j])for j in range(0,d) ] for i in range(0,d)])
    A=A_1+B; 
...
# |------|:----------|:--------:|:-----------:|:--------:|
# |  3   |5.0000e-1  |2.5000e-1 | 8.3459e-2   |   ----   |
# |      |2.5000e-1  |6.2500e-2 | 9.4976e-3   |  1.5677  | 
# |      |2.0000e-1  |4.0000e-2 | 4.6388e-3   |  1.6057  |  
# |      |1.2500e-1  |1.5625e-2 | 1.1684e-3   |  1.4668  |   
# |      |1.0000e-1  |1.0000e-2 | 5.9047e-4   |  1.5292  | 
...
# |  r   | h         |  k= h^2  |  L^2 norm   |   rate   |  
# |------|:----------|:--------:|:-----------:|:--------:|
# |  4   |5.0000e-1  |2.5000e-1 | 8.2612e-3   |   ----   |
# |      |2.5000e-1  |6.2500e-2 | 4.8451e-4   |  2.0459  | 
# |      |2.0000e-1  |4.0000e-2 | 1.8660e-4   |  2.1380  |  
# |      |1.2500e-1  |1.5625e-2 | 2.9049e-5   |  1.9787  |   
# |      |1.0000e-1  |1.0000e-2 | 1.1718e-5   |  2.0342  | 

# T=1, tol=1e-10 CG5 in space k=h^2--> expected order O(k^{5/2})

# |  r   | h         |  k= h^2  |  L^2 norm   |   rate   |  
# |------|:----------|:--------:|:-----------:|:--------:|
# |  5   |5.0000e-1  |2.5000e-1 | 6.6916e-4   |   ----   |
# |      |2.5000e-1  |6.2500e-2 | 1.8487e-5   |  2.5888  | 
# |      |2.0000e-1  |4.0000e-2 | 5.7761e-6   |  2.6067  |  
# |      |1.2500e-1  |1.5625e-2 | 5.6859e-7   |  2.4663  |   
# |      |1.0000e-1  |1.0000e-2 | 1.8397e-7   |  2.5284  | 


# T=1, tol=1e-10 CG6 in space k=h^2--> expected order O(k^3)

# |  r   | h         |  k= h^2  |  L^2 norm   |   rate   |  
# |------|:----------|:--------:|:-----------:|:--------:|
# |  6   |5.0000e-1  |2.5000e-1 | 6.7489e-5   |   ----   |
# |      |2.5000e-1  |6.2500e-2 | 6.8508e-7   |  3.3111  | 
# |      |2.0000e-1  |4.0000e-2 | 1.6209e-7   |  3.2297  |  
# |      |1.2500e-1  |1.5625e-2 | 9.5069e-9   |  3.0171  |   
# |      |1.0000e-1  |1.0000e-2 | 2.4616e-9   |  3.0277  | 
...
error_3= np.array([ 8.3459e-2, 9.4976e-3  ,4.6388e-3 , 1.1684e-3, 5.9047e-4]);
error_4= np.array([ 8.2612e-3, 4.8451e-4  ,1.8660e-4, 2.9049e-5  , 1.1718e-5  ]);
error_5= np.array([ 6.6916e-4 , 1.8487e-5, 5.7761e-6  ,  5.6859e-7,  1.8397e-7  ]);
error_6= np.array([ 6.7489e-5 , 6.8508e-7, 1.6209e-7 ,  9.5069e-9 ,  2.4616e-9  ]);
loglog(Delta_t, error_3, label='CG 3', linestyle='-', marker='o', color='blue', linewidth=3)
loglog(Delta_t, error_4, label='CG 4', linestyle=':', marker='p', color='red', linewidth=3)
loglog(Delta_t, error_5, label='CG 5', linestyle='--',marker='D', color='black', linewidth=3)
loglog(Delta_t, error_6, label='CG 6', linestyle='--',marker='x', color='green', linewidth=3)
title('Plot of $||u- u_\mathrm{DG}||_{H^1(\Omega)}+||\dot{u}- \dot{u}_\mathrm{DG}||_{L^2(\Omega)}$ versus 1/k for DG3 in time')
legend(loc = 'best')
grid()
savefig('1D_nonlinear_wave_DG3_L2error.png', dpi=300, bbox_inches='tight')
