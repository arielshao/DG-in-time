...
for it in range(1,nt):
    ...
    B=np.block([[L[i, j]*M2+K0[i,j]*(M3+M5)+np.multiply(K11_bar[i,j],M3_bar[0][0])+np.multiply(K22_bar[i,j],M3_bar[1][1])+np.multiply(K33_bar[i,j],M3_bar[2][2])+np.multiply(K12_bar[i,j],M3_bar[0][1])+np.multiply(K23_bar[i,j],M3_bar[1][2])+np.multiply(K13_bar[i,j],M3_bar[0][2])+np.multiply(M2_bar[0][0],M11_bar[i,j])+np.multiply(M2_bar[1][1],M22_bar[i,j])\
    +np.multiply(M2_bar[2][2],M33_bar[i,j]) + np.multiply(M2_bar[0][1],M12_bar[i,j])+np.multiply(M2_bar[1][2],M23_bar[i,j])\
    +np.multiply(M2_bar[0][2],M13_bar[i,j])for j in range(0,d)] for i in range(0,d)])
    A=A_1+B;
    b[0::Dt]= (K0@Z_minus[it-1,:])
    b[1::Dt]=-(K0@Z_minus[it-1,:])+Zprime_minus[it-1,:]*2/k+quad(expt1, time[it-1], time[it])[0]*Gexpt1+quad(expt1_3, time[it-1], time[it])[0]*Gexpt3
    b[2::Dt]= (K0@Z_minus[it-1,:])+Zprime_minus[it-1,:]*(-6/k)+quad(expt2, time[it-1], time[it])[0]*Gexpt1+quad(expt2_3, time[it-1], time[it])[0]*Gexpt3   
...
# |------|:----------|:--------:|:-----------:|:--------:|
# |  2   |5.0000e-1  |2.5000e-1 | 6.0903e-1   |   ----   |
# |      |2.5000e-1  |6.2500e-2 | 1.4774e-1   |  1.0217  | 
# |      |2.0000e-1  |4.0000e-2 | 8.9097e-2   |  1.1332  |  
# |      |1.2500e-1  |1.5625e-2 | 3.5502e-2   |  0.9789  |   
# |      |1.0000e-1  |1.0000e-2 | 2.2374e-2   |  1.0345  | 
...
# |  r   | h         |  k= h^2  |  L^2 norm   |   rate   |  
# |------|:----------|:--------:|:-----------:|:--------:|
# |  3   |5.0000e-1  |2.5000e-1 | 8.5172e-2   |   ----   |
# |      |2.5000e-1  |6.2500e-2 | 9.5072e-3   |  1.5816  | 
# |      |2.0000e-1  |4.0000e-2 | 4.6403e-3   |  1.6072  |  
# |      |1.2500e-1  |1.5625e-2 | 1.1684e-3   |  1.4672  |   
# |      |1.0000e-1  |1.0000e-2 | 5.9048e-4   |  1.5292  | 
...
# |  r   | h         |  k= h^2  |  L^2 norm   |   rate   |  
# |------|:----------|:--------:|:-----------:|:--------:|
# |  4   |5.0000e-1  |2.5000e-1 | 1.2530e-2   |   ----   |
# |      |2.5000e-1  |6.2500e-2 | 5.4227e-4   |  2.2651  | 
# |      |2.0000e-1  |4.0000e-2 | 2.0021e-4   |  2.2326  |  
# |      |1.2500e-1  |1.5625e-2 | 2.9801e-5   |  2.0264  |   
# |      |1.0000e-1  |1.0000e-2 | 1.1899e-5   |  2.0572  | 
...
error_2= np.array([ 6.0903e-1, 1.4774e-1  , 8.9097e-2 , 3.5502e-2, 2.2374e-2]);
error_3= np.array([  8.5172e-2 , 9.5072e-3  , 4.6403e-3 , 1.1684e-3 , 5.9048e-4  ]);
error_4= np.array([ 1.2530e-2 , 5.4227e-4, 2.0021e-4  ,  2.9801e-5,  1.1899e-5  ]);
...
title('Plot of $||u- u_\mathrm{DG}||_{H^1(\Omega)}+||\dot{u}- \dot{u}_\mathrm{DG}||_{L^2(\Omega)}$ versus 1/k for DG2 in time')
legend(loc = 'best')
grid()
savefig('1D_nonlinear_wave_DG2_L2error.png', dpi=300, bbox_inches='tight')
