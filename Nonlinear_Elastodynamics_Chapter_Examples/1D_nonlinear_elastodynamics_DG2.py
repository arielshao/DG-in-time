...
A2=Expression("pi*cos(pi*x[0])",degree=10)
A3=Expression("pi*cos(pi*x[0])",degree=10)
...
for it in range(1,nt):
    ...
    +K13_bar[i,j]*(M3_bar[0][2]+M5_bar[0][2]) for j in range(0,d) ] for i in range(0,d)])
    A=A_1+B;
    ...
    -(2*phi(time[it-1],0)*phi(time[it-1],1))*(K12_bar@Z_minus[it-1,:])-(2*phi(time[it-1],1)*phi(time[it-1],2))*(K23_bar@Z_minus[it-1,:])-(2*phi(time[it-1],2)*phi(time[it-1],0))*(K13_bar@Z_minus[it-1,:])+Zprime_minus[it-1,:]*2/k+quad(sint1, time[it-1], time[it])[0]*Gsin+quad(cost1, time[it-1], time[it])[0]*Gcos+quad(sint1_3, time[it-1], time[it])[0]*Gsin3
    ...
    +(2*phi(time[it-1],0)*phi(time[it-1],1))*(K12_bar@Z_minus[it-1,:])+(2*phi(time[it-1],1)*phi(time[it-1],2))*(K23_bar@Z_minus[it-1,:])+(2*phi(time[it-1],2)*phi(time[it-1],0))*(K13_bar@Z_minus[it-1,:])+Zprime_minus[it-1,:]*(-6/k)+quad(sint2, time[it-1], time[it])[0]*Gsin+quad(cost2, time[it-1], time[it])[0]*Gcos+quad(sint2_3, time[it-1], time[it])[0]*Gsin3   
    eps =1.0
    tol=1e-10
    maxiter= 30
    iter =0
    ...
    print ('''
        ...
        %s
        ''' % ( convergence))
    integrand=lambda t: 2*gamma*np.dot(alpha[1::Dt]*phi_t(t,1)+alpha[2::Dt]*phi_t(t,2)-Zprime_e(t), alpha[1::Dt]*phi_t(t,1)+alpha[2::Dt]*phi_t(t,2)-Zprime_e(t))
    ...
    error_energy=1/2*(Zprime_plus[it-1,:]-Zprime_minus[it-1,:])@(Zprime_plus[it-1,:]-Zprime_minus[it-1,:])+1/2*(K0@(Z_plus[it-1,:]-Z_minus[it-1,:]))@(Z_plus[it-1,:]-Z_minus[it-1,:])+1/2*(phi(time[it-1],0)**2)*(K11_bar@(Z_plus[it-1,:]-Z_minus[it-1,:]))@(Z_plus[it-1,:]-Z_minus[it-1,:])+1/2*(phi(time[it-1],1)**2)*(K22_bar@(Z_plus[it-1,:]-Z_minus[it-1,:]))@(Z_plus[it-1,:]-Z_minus[it-1,:])+1/2*(phi(time[it-1],2)**2)*(K33_bar@(Z_plus[it-1,:]-Z_minus[it-1,:]))@(Z_plus[it-1,:]-Z_minus[it-1,:])+(phi(time[it-1],0)*phi(time[it-1],1))*(K12_bar@(Z_plus[it-1,:]-Z_minus[it-1,:]))@(Z_plus[it-1,:]-Z_minus[it-1,:])+(phi(time[it-1],1)*phi(time[it-1],2))*(K23_bar@(Z_plus[it-1,:]-Z_minus[it-1,:]))@(Z_plus[it-1,:]-Z_minus[it-1,:])+(phi(time[it-1],0)*phi(time[it-1],2))*(K13_bar@(Z_plus[it-1,:]-Z_minus[it-1,:]))@(Z_plus[it-1,:]-Z_minus[it-1,:])+quad(integrand,time[it-1], time[it])[0] +error_energy
...
+1/2*(phi(time[nt-1],0)**2)*(K11_bar@(Z_minus[nt-1,:]-Z_e(time[nt-1])))@(Z_minus[nt-1,:]-Z_e(time[nt-1]))+1/2*(phi(time[nt-1],1)**2)*(K22_bar@(Z_minus[nt-1,:]-Z_e(time[nt-1])))@(Z_minus[nt-1,:]-Z_e(time[nt-1]))+1/2*(phi(time[nt-1],2)**2)*(K33_bar@(Z_minus[nt-1,:]-Z_e(time[nt-1])))@(Z_minus[nt-1,:]-Z_e(time[nt-1]))\
+(phi(time[nt-1],0)*phi(time[nt-1],1))*(K12_bar@(Z_minus[nt-1,:]-Z_e(time[nt-1])))@(Z_minus[nt-1,:]-Z_e(time[nt-1]))+(phi(time[nt-1],0)*phi(time[nt-1],2))*(K13_bar@(Z_minus[nt-1,:]-Z_e(time[nt-1])))@(Z_minus[nt-1,:]-Z_e(time[nt-1]))+(phi(time[nt-1],1)*phi(time[nt-1],2))*(K23_bar@(Z_minus[nt-1,:]-Z_e(time[nt-1])))@(Z_minus[nt-1,:]-Z_e(time[nt-1]))
...
# T=1, gamma=1 tol=1e-10  CG2 in space k=h^2, MaxIteration=30

# | p=q  |    h     |   k= h^2  |  L^2 norm  |  rate  |
# |------|:--------:|:---------:|:----------:|: -----:|
# |      |2.5000e-1 |6.2500e-2  | 1.2123e-2  | 2.0030 |   
# |      |2.0000e-1 |4.0000e-2  | 4.9774e-3  | 1.9948 |  
# |      |1.2500e-1 |1.5625e-2  | 1.1643e-3  | 1.5455 |   
# |      |6.2500e-2 |3.90625e-3 | 1.2454e-4  | 1.6124 | 